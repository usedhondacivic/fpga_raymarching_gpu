...
volatile unsigned int *repetition_pow;
...
void a_walk_through_pretty_lights(){
	*red_shift = 5;
	*color_enables |= 1;
	*green_shift = 5;
	*color_enables |= 1 << 1;
...
void horror_movie(){
	...
	*blue_shift = 3;
	*fog_shift = 2;
	*color_enables = 0b1001;
	fly_random(0.2, 0.05, 0.015);
}
...
void like_a_fish(){
	...
	*blue_shift = 3;
	*fog_shift = 2;
	*color_enables = 0b11110;
	fly_random(0.02, 0.005, 0.001);
}
...
void beating_heart(){
	...
	*blue_shift = 3;
	*fog_shift = 2;
	*color_enables = 0b11101;
	fly_random(0.09, 0.05, 0.001);
...
void fast_party(){
	...
	*blue_shift = 3;
	*fog_shift = 2;
	*color_enables = 0b1111;
	fly_random(0.34, 0.005, 0.001);
}
...
void *visual_thread(void *args){
	...
	repetition_pow=(unsigned int *)(h2p_lw_virtual_base + (( REPETITION_POW ) & ( HW_REGS_MASK ) ));
	*repetition_pow = 5;
	enum MOOD mood = ((arg *)args)->mood;
	float energy = ((arg *)args)->energy;
	while(1){
		if( mood == ANGRY) {
			if(energy > 0.5){
				horror_movie();
...
// 	iret = pthread_create( &thread, NULL, visual_thread, (void*) &pthread_args);
